...
{   char tmp[8192];
    ...
    f = popen(tmp, "r");
    if (!f) {
        printf("%s\n", "unable to disassembly given binary argument");
        return (-1);
    }
    ...
    while ( fgets ( tmp , 8191, f ) ) {
        ...
        if (e) {
...
#else
            {
                ...
                if (s) {
                    /* clean, in case instruction is without args, i.e. nop */
                    x = s;
                    while(*x) {
                        if (*x == '\n' || *x == '\r')
                            *x = 0;
                    ...
                    if (!ok) {
                        for (i = 0; i < NUMAES; i++) {
                            if (!strcmp(s, setaes[i])) { ok = 1; iaes++; break; }
                        }
                    }
                    
                    if (!ok) {
                        for (i=0; i<NUMPCLMUL;i++) {
                            if(!strcmp(s, setpclmul[i])) { ok = 1; ipclmul++; break; }
                        }
                    }

                    if (!ok) {
                        for (i=0; i<NUMAVX;i++) {
                            if(!strcmp(s, setavx[i])) { ok = 1; iavx++; break; }
                        }
                    }
                } /* instruction */
            } /* strtok2: hex */
